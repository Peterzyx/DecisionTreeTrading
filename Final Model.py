    ...
    if len(context.spy) == context.window_length+1: # If there's enough recent price data
        ...
        for single_security in context.output.index.tolist():
            ...
            if single_security in context.output.index and len(context.recent_prices[single_security].index) == context.window_length+2:
                ...
                returns_1 = context.recent_prices[single_security].iloc[-1]['returns_1']
                change = returns_1 > benchmark and returns_1 > 0

                row = {}
                row['RSI'] = context.recent_prices[single_security].iloc[1]['RSI']
                row['EMA'] = context.recent_prices[single_security].iloc[1]['EMA']
                row['MACD'] = context.recent_prices[single_security].iloc[1]['MACD']
                row['SMA_5'] = context.recent_prices[single_security].iloc[1]['SMA_5']
                row['SMA_10'] = context.recent_prices[single_security].iloc[1]['SMA_10']
                row['bb_lower'] = context.recent_prices[single_security].iloc[1]['bb_lower']
                row['bb_middle'] = context.recent_prices[single_security].iloc[1]['bb_middle']
                row['bb_upper'] = context.recent_prices[single_security].iloc[1]['bb_upper']
                ...
                res = {}
                res['Class'] = change
                if len(context.Y.index) >= 8500:
                    context.X = context.X.iloc[1:]
                    context.Y = context.Y.iloc[1:]
        ...
        if len(context.Y.index) >= 200: # There needs to be enough data points to make a good model and adjust positions once per 5 days
            #if context.count <= 1:
